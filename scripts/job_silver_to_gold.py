...
Lee los datos limpios de Silver y construye un modelo estrella en Gold:
- dim_product
- dim_customer
- dim_date
- fact_sales
...
try:
    ...
    logger.info(f"STEP 2 - Read succeeded. Rows read from silver/: {silver_count}")
except Exception as error:
    logger.error(f"STEP 2 - Failed to read from silver/: {str(error)}")
    raise
...
try:
    ...
    logger.info(f"STEP 3 - dim_product built. Rows: {dim_product.count()}")
except Exception as error:
    logger.error(f"STEP 3 - Failed to build dim_product: {str(error)}")
    raise
...
try:
    ...
    logger.info(f"STEP 4 - dim_customer built. Rows: {dim_customer.count()}")
except Exception as error:
    logger.error(f"STEP 4 - Failed to build dim_customer: {str(error)}")
    raise
...
try:
    ...
    logger.info(f"STEP 5 - dim_date built. Rows: {dim_date.count()}")
except Exception as error:
    logger.error(f"STEP 5 - Failed to build dim_date: {str(error)}")
    raise

# --- Step 6: Build fact_sales with broadcast joins ---
try:
    fact = (
    ...
    logger.info(f"STEP 6 - fact_sales built. Rows: {fact_count}")
except Exception as error:
    logger.error(f"STEP 6 - Failed to build fact_sales: {str(error)}")
    raise
...
try:
    ...
    logger.info(f"STEP 7 - Write succeeded. Target: {GOLD_PATH}")
except Exception as error:
    logger.error(f"STEP 7 - Failed to write to gold/: {str(error)}")
    raise
